 ...
 * job is a purely cosmetic `y` transform: the header visually starts near
 * the vertical middle of the viewport and scrubs up to `y: 0` (its fixed
 * `top: 0`) as the user scrolls the first ~420px of the Hero. Once the scrub
...
export function stickyHeader(
  ...
  const startY = Math.round(window.innerHeight * 0.38);
